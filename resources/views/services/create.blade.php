                ...
                <ol class="inline-flex items-center space-x-1 md:space-x-3">
                    ...
                    <li>
                        <div class="flex items-center">
                            ...
                            <span class="ml-1 text-xs font-bold text-indigo-600 uppercase tracking-widest">{{ __('Add New') }}</span>
                        </div>
                    </li>
                </ol>
            </nav>
        ...
        <form action="{{ route('services.store') }}" method="POST">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
                <div class="col-span-full md:col-span-1">
                    ...
                    <input type="text" name="name" id="name" value="{{ old('name') }}" class="w-full rounded-2xl border-slate-200 focus:border-indigo-500 focus:ring-indigo-500 shadow-sm" required>
                    @error('name') <span class="text-red-500 text-xs mt-1">{{ $message }}</span> @enderror
                </div>

                <div class="col-span-full md:col-span-1">
                    <label for="price" class="block text-sm font-bold text-slate-700 mb-2">{{ __('Price (€)') }}</label>
                    <input type="number" step="0.01" name="price" id="price" value="{{ old('price') }}" class="w-full rounded-2xl border-slate-200 focus:border-indigo-500 focus:ring-indigo-500 shadow-sm" required>
                    @error('price') <span class="text-red-500 text-xs mt-1">{{ $message }}</span> @enderror
                </div>

                <div class="col-span-full md:col-span-1">
                    <label for="duration" class="block text-sm font-bold text-slate-700 mb-2">{{ __('Duration (minutes)') }}</label>
                    <input type="number" name="duration" id="duration" value="{{ old('duration') }}" class="w-full rounded-2xl border-slate-200 focus:border-indigo-500 focus:ring-indigo-500 shadow-sm" required>
                ...
                <div class="col-span-full">
                    ...
                    <textarea name="description" id="description" rows="4" class="w-full rounded-2xl border-slate-200 focus:border-indigo-500 focus:ring-indigo-500 shadow-sm" required>{{ old('description') }}</textarea>
